...
Despite advances in OCR technology, historical documents remain difficult to digitize accurately. Most institutions lack the resources to re-OCR millions of legacy pages, making **post-correction the primary path to improving OCR quality at scale**. The rise of LLMs has opened new possibilities, but results across languages and error types remain inconsistent and hard to compare.
...
**`impresso-snippets`** — Short newspaper paragraphs sampled from the multilingual Impresso corpus, and newly transcribed. Line information. No sentence or chunk segmentation. New splits created for this benchmark.
...
- **soft hyphens** (`¬\n`): only present where the original data had line-break hyphenation information, or where newly introduced during curation.
...
| `impresso-snippets` | fr | various                                                        | TBD | TBD | TBD | TBD | manual (Impresso) | newly created |
| `impresso-snippets` | de | various                                                        | TBD | TBD | TBD | TBD | manual (Impresso) | newly created |
...
> In test files, `ground_truth` is masked (`null`). The `cer` and `wer` fields are pre-computed on training/dev data and provided as indicative quality signals only. 
...
A score of +1 means the system improved over the input OCR, 0 means no change, −1 means degradation. The mean preference score captures the *consistency* of improvement across documents, complementing the *magnitude* measured by cMER.
